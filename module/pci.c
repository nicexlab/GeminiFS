...
static int nvme_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	printk("%s: ioq_num %d, ioq_map_num %d\n", __func__, ctrl->ioq_num, ctrl->ioq_map_num);
...
static const struct pci_device_id snvme_id_table[] = {
...
MODULE_DEVICE_TABLE(pci, snvme_id_table);
...
static long snvm_dev_map_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
		...
		case NVM_SET_IOQ_NUM:
		{
			...
			printk("%s: NVM_SET_IOQ_NUM %d\n", __func__, request.ioq_idx);
...
static int snvm_create_cdev_for_device(struct pci_dev *pdev, unsigned int class){
	struct ctrl* ctrl = NULL;
	int err;
	if (pdev->class != class) {
			      ...
			      unsigned long arg)
{
	...
	struct device_driver *dev_drv = NULL, *snvm_drv = NULL;
	...
	switch (cmd)
	{
        case SNVM_REGISTER_DRIVER:
        {
			...
			if (snvm_registered){
				printk(KERN_NOTICE "snvme driver has registered\n");
				ret = 0;
			}else{
				printk(KERN_NOTICE "driver result: %d\n", ret);
		...
		case SNVM_DEVICE_BIND:{
			printk("SNVM_DEVICE_BIND\n");
			if (copy_from_user(&dev_addr, (void __user*) arg, sizeof(dev_addr))){
				return -EFAULT;
			}
			pdev = pci_get_domain_bus_and_slot(0, dev_addr.bus, PCI_DEVFN(dev_addr.slot, dev_addr.func));
			if (pdev){
				// unbind device driver if exist
				dev_drv = pdev->dev.driver;
				if (dev_drv){
					if(dev_drv->name){
						if (strcmp(dev_drv->name, "snvme") == 0){
							printk("device already bind\n");
							return 0;
						}else{
							// unbind
							if (pci_is_enabled(pdev)){
								pci_disable_device(pdev);
								printk("disable device for bind new driver\n");
							}
							device_release_driver(&pdev->dev);
						}
					}else{
						printk("device without driver\n");
					}
				}

				printk("start to bind nvme device to snvme: pci %x:%x.%x\n",
							dev_addr.bus, 
							dev_addr.slot, 
							dev_addr.func);
				snvm_drv = driver_find("snvme", &pci_bus_type);
				mutex_lock(&snvm_control_lock);
				if (!snvm_drv){
					ret = snvm_register_driver();
					if (ret){
		...
		case SNVM_SET_CDEV:{
			...
			ctrl = ctrl_find_by_pci_dev(&ctrl_list, pdev);
			if (!ctrl){
				ret = snvm_create_cdev_for_device(pdev,PCI_CLASS_STORAGE_EXPRESS);
				printk("cdev result %d\n", ret);
			}else{
				printk("find ctrl\n");
			}
			return ret;
		}
		case SNVM_CLEAR_CDEV:{
			printk("SNVM_CLEAR_CDEV\n");
			if (copy_from_user(&dev_addr, (void __user*) arg, sizeof(dev_addr))){
				return -EFAULT;
			}
			pdev = pci_get_domain_bus_and_slot(0, dev_addr.bus, PCI_DEVFN(dev_addr.slot, dev_addr.func));
...
static void __exit nvme_exit(void)
{
	...
	if (snvm_registered){
		snvm_unregister_driver();
		printk("unregister snvme driver\n");
	}
	dev_drv = driver_find("snvme", &pci_bus_type);
	if (dev_drv){
		driver_unregister(dev_drv);
	}
	mutex_unlock(&snvm_control_lock);
