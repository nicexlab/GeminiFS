...
out_stop_queue:
	nvme_tcp_stop_queue(ctrl, 0);
	snvme_cancel_admin_tagset(ctrl);
out_cleanup_queue:
	if (new)
		blk_cleanup_queue(ctrl->admin_q);
...
static void snvme_reset_ctrl_work(struct work_struct *work)
{
	struct nvme_ctrl *ctrl =
		...
		struct nvmf_ctrl_options *opts)
{
	...
	INIT_WORK(&ctrl->ctrl.reset_work, snvme_reset_ctrl_work);
